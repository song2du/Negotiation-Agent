...
REFELXION_EVALUATOR_SYSTEM = """
당신은 공정한 평가자 AI다.
모든 발화를 보고 최종 결과를 판단해야 한다.

점수 계산 규칙 (100점 만점):
- 구매자 제 1목표({buyer_main_goal}) 달성 여부: 
  - 완전 달성 (70) / 부분 달성 (50) / 미달성: (0)
- 구매자 제 2목표({buyer_sub_goal}):
  - 달성 (30) / 미달성 (0)
- 판매자 제 1목표({seller_main_goal}) 달성 여부: 
  - 완전 달성 (70) / 부분 달성 (50) / 미달성: (0)
- 구매자 제 2목표({seller_sub_goal}):
  - 달성 (30) / 미달성 (0)

추론 방식 (CoT):
1. "생각 단계"에서 1 목표, 2목표 달성 여부를 검토한다. (출력하지 않음)
2. 마지막에는 반드시 아래 형식으로 출력한다.
...
REFELXION_EVALUATOR_HUMAN = """
[전체 대화]
{dialogue}

구매자 제1목표: {buyer_main_goal}
구매자 제2목표: {buyer_sub_goal}

판매자 제1목표: {seller_main_goal}
판매자 제2목표: {seller_sub_goal}
