...
#                 f"목표 {idx+1}", value=goal_name,
#                 key=f"{role}_{key_prefix}_name_{idx}",
#                 help="목표의 내용을 수정할 수 있습니다."
#             )
#         with col2:
#             new_score = st.number_input(
#                 "배점", min_value=0, max_value=100, value=score, step=5,
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            if st.button("🚀 협상 시작하기", use_container_width=True, type="primary"):
                # 기본값 설정 (주석처리된 역할·모델·우선순위 대신 사용)
                role = "구매자"
                model_name = "gpt-4o"
                user_goals_dict = PRIORITIES.get(role, {})
                ai_goals_dict = PRIORITIES.get("판매자", {})
